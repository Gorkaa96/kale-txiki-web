...
export default function MenuList({ menus, code }: { menus: Menu[]; code: string }) {
  ...
  return <aside className={styles.darkCard}><div className={styles.kicker}>Menús</div><h2>Panel rápido</h2><p className={styles.panelHint}>Publica lo que quieras mostrar en la web. Usa “Duplicar” para preparar un menú nuevo a partir de uno anterior.</p>{menus.length === 0 ? <div className={styles.empty}>Aún no hay menús guardados.</div> : <div className={styles.sections}><MenuSection title="Publicados" menus={publicados} code={code} /><MenuSection title="Borradores" menus={borradores} code={code} /><MenuSection title="Archivados" menus={archivados} code={code} /></div>}</aside>;
}
